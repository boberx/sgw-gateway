 ...
 */
s_config* config_get_config () { return &config; }
...
void config_init ()
{
    ...
    config.checkinterval = rand () % ( DEFAULT_CHECKINTERVAL_MAX - DEFAULT_CHECKINTERVAL_MIN ) + DEFAULT_CHECKINTERVAL_MIN;
    config.daemon = -1;
    config.pidfile = NULL;
...
void config_reload ( const char* filename )
{
	FILE* fd;
	char line[MAX_BUF], *s, *p1, *p2, *rawarg = NULL;
...
//	s_config *config;
	...
	debug ( LOG_INFO, "Reload configuration file '%s'", filename );

//	config = config_get_config ();

	if ( ! ( fd = fopen ( filename, "r" ) ) )
	{
		debug ( LOG_ERR, "Could not open configuration file '%s', " "return...", filename );
		return;
	}

	/* clean config */
	t_trusted_mac* head = config.trustedmaclist;
	t_trusted_mac* curr = NULL;
		...
		if ( p1 && p1[0] != '\0' )
		{
			if ( ( strncmp ( s, "#", 1 ) ) != 0 )
			{
				...
				switch ( opcode )
				{
					...
					break;
					default:
						debug ( LOG_DEBUG, "not processed option on line %d", linenum );
					break;
				}
			}
		}
