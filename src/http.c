...
http_callback_404(httpd * webserver, request * r, int error_code)
{
    ...
    if (!is_online()) {
        ...
        if (!(mac = arp_get(r->clientAddr))) {
			...
			safe_asprintf (
				&urlpreauth,
				"%sstage=preauth&gw_address=%s&gw_port=%d&gw_id=%s&ip=%s&mac=%s&url=%s",
		...
		/*
		*	запрос преаутентификации
		*/
		if ( urlpreauth != 0 && config->preauth == 1 )
		{
			debug ( LOG_INFO, "Pre-Authentication for: %s", r->clientAddr );
			http_get_preauth ( r, urlpreauth );
		}
...
void http_get_preauth ( request* r, const char* urlFragment )
{
	char* protocol = NULL;
	int port = 80;
	t_auth_serv* auth_server = get_auth_server ();
	...
	auth_server_request_preauth ( &authresponse, urlFragment );

	if ( authresponse.authcode == AUTH_ERROR )
		debug ( LOG_WARNING, "Auth server error when pre-authentication" );
...
void send_http_page ( request* r, __attribute__((unused)) const char* title, __attribute__((unused)) const char* message )
{
	s_config* config = config_get_config ();
    ...
    const char* buffer =
        ...
        "<style>#header{height: 30px;padding: 20px;font-size: 20pt;text-align: center;border-bottom: 0;}</style>"
        "</head>"
        "<body>"
        "<div id='header'><h2>Service is temporarily unavailable. Please try later.</h2></div>"
        "</body>"
        "</html>";
