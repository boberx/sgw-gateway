...
t_authcode auth_server_request_preauth ( t_authresponse* authresponse, const char* request )
{
	int sockfd;
	char buf[MAX_BUF];
	char* tmp;
	...
	if ( ( tmp = strstr ( res, "Auth: ") ) )
	{
		if ( sscanf ( tmp, "Auth: %d", (int *)&authresponse->authcode) == 1 )
		{
			debug ( LOG_INFO, "Auth server returned pre-authentication code %d", authresponse->authcode );
			free ( res );
			return ( authresponse->authcode );
		}
		else
		{
			debug ( LOG_WARNING, "Auth server did not return expected pre-authentication code" );
			free ( res );
			return ( AUTH_ERROR );
		}
	}
