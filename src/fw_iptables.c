...
iptables_do_command(const char *format, ...)
{
    ...
    safe_asprintf(&cmd, "iptables -w %d %s", iptables_wait, fmt_cmd);
    free(fmt_cmd);
...
int iptables_fw_reinit ( void )
{
	...
	for ( p = config->trustedmaclist; p != NULL; p = p->next )
		iptables_do_command (
			"-t mangle -A " CHAIN_TRUSTED " -m mac --mac-source %s -j MARK --set-mark %d",
			p->mac,
			FW_MARK_KNOWN );
...
iptables_fw_init(void)
{
    ...
    iptables_do_command("-t nat -A " CHAIN_TO_INTERNET " -m mark --mark 0x%u -j ACCEPT", FW_MARK_WIFLY);
    ...
    iptables_do_command("-t filter -A FORWARD -i %s -j " CHAIN_TO_INTERNET, config->gw_interface);
	...
	iptables_do_command("-t filter -A " CHAIN_TO_INTERNET " -m mark --mark 0x%u -j " CHAIN_KNOWN, FW_MARK_WIFLY);
...
iptables_fw_destroy_mention(const char *table, const char *chain, const char *mention)
{
    ...
    safe_asprintf(&command, "iptables -w %d -t %s -L %s -n --line-numbers -v", iptables_wait, table, chain);
...
iptables_fw_counters_update(void)
{
    ...
    safe_asprintf(&script, "%s -w %d %s", "iptables", iptables_wait, "-v -n -x -t mangle -L " CHAIN_OUTGOING);
    ...
    safe_asprintf(&script, "%s -w %d %s", "iptables", iptables_wait, "-v -n -x -t mangle -L " CHAIN_INCOMING);
    iptables_insert_gateway_id(&script);
    output = popen(script, "r");
    free(script);
    if (!output) {
